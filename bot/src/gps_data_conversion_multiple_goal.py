...
def get_xy_based_on_lat_long(msg,distance_pub):
	...
	quaternion = tf.transformations.quaternion_from_euler(0.0,0.0,p.theta)#incomplete
...
def entered_lat_long_function(msg):
  global d_lat, d_lon
  d_lat=msg.lat
  d_lon=msg.long
  print(d_lat)
...
if __name__ == '__main__':
	...
	entered_lat_long_sub=rospy.Subscriber('/Multiple_goal_lat_long_topic', lat_long_msg, entered_lat_long_function)	
  # print("Enter the Goal Location GPS coordinates")
	# d_lat=float(input("Enter Latitude:"))
	# d_lon=float(input("Enter Longitude:"))
	distance_pub=rospy.Publisher('/distance',Pose,queue_size=50)
	rospy.Subscriber('/gps_fix',NavSatFix,get_xy_based_on_lat_long,distance_pub)
